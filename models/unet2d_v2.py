...
class SpatialFFN(nn.Module):
    """
    Spatial Feed-Forward Network using SwiGLU gating for images.
    Combines SwiGLU (SiLU gating) for channel mixing with depthwise spatial conv.
    """
    def __init__(
        self,
        in_channels: int,
        ...
        super().__init__()
        # Compute hidden channels
        hid = int(2 * hidden_dim / 3)
        if ffn_dim_multiplier is not None:
            hid = int(ffn_dim_multiplier * hid)
        hid = multiple_of * ((hid + multiple_of - 1) // multiple_of)

        # SwiGLU channel projections
        self.gate_proj = nn.Conv2d(in_channels, hid, kernel_size=1, bias=False)
        self.value_proj = nn.Conv2d(in_channels, hid, kernel_size=1, bias=False)
        # Depthwise conv for spatial mixing
        self.dw_conv = nn.Conv2d(hid, hid, kernel_size=kernel_size,
                                 padding=padding, groups=hid, bias=False)
        # Output projection back to in_channels
        self.out_proj = nn.Conv2d(hid, in_channels, kernel_size=1, bias=False)
        # Optional dropout
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        hidden = F.silu(gate) * value  # (B, hid, H, W)
        # Spatial mixing
        hidden = self.dw_conv(hidden)
        hidden = self.dropout(hidden)
        # Project back
        out = self.out_proj(hidden)
        return out

class RotaryLinformerBlock(nn.Module):
    def __init__(self, dim, n_heads, n_kv_heads):
        super().__init__()
        self.n_heads = n_heads
        self.n_kv_heads = n_kv_heads
        self.head_dim = dim // n_heads
        self.attn = RotaryLinearAttention(dim, n_heads, n_kv_heads, use_rmsnorm=False)
        self.ffn = SpatialFFN(dim, dim, multiple_of=8, ffn_dim_multiplier=2.0, drop=0.1)
        self.attn_norm = RMSNorm(dim)
        self.ffn_norm = RMSNorm(dim)
    
    def forward(self, x):
        h = x + self.attn_norm(self.attn(x))
        h = h + self.ffn_norm(self.ffn(h))
        return h

class SpatialRotaryLinformer(nn.Module):
    def __init__(self, dim, n_heads, n_kv_heads, depth=2):
        super().__init__()
        self.depth = depth
        self.blocks = nn.ModuleList([RotaryLinformerBlock(dim, n_heads, n_kv_heads) for _ in range(depth)])
...
class DownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, time_dim, use_attn=False, cond_dim=1, attn_depth=1):
        super().__init__()
        ...
        if use_attn:
            ...
            self.attn = SpatialRotaryLinformer(in_channels, n_heads=16, n_kv_heads=16, depth=self.attn_depth)
...
class RotaryUNet(nn.Module):
    def __init__(
        ...
        self.down_blocks.append(DownBlock(ch*1, ch*2, time_dim, use_attn=True, attn_depth=1))
        self.down_blocks.append(DownBlock(ch*2, ch*4, time_dim, use_attn=True, attn_depth=1))
        self.down_blocks.append(DownBlock(ch*4, ch*8, time_dim, use_attn=True, attn_depth=1))
        ...
        self.mid_attn = SpatialRotaryLinformer(ch*8, n_heads=16, n_kv_heads=16, depth=1)
        # self.mid_attn = nn.Identity()

        # Up blocks - decreasing channel dimensions
        self.up_blocks = nn.ModuleList()

        self.up_blocks.append(UpBlock(ch*16, ch*4, time_dim, use_attn=True, attn_depth=1))
        self.up_blocks.append(UpBlock(ch*8, ch*2, time_dim, use_attn=True, attn_depth=1))
        self.up_blocks.append(UpBlock(ch*4, ch*1, time_dim, use_attn=True, attn_depth=1))
