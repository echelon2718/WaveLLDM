...
class DiraconDistribution(AbstractDistribution):
...
class DiagonalGaussianDistribution(object):
    def __init__(self, params, deterministic=False):
        ...
        if self.deterministic:
            self.var = self.std = torch.zeros_like(self.mean).to(device = self.params.device)
    
    def sample(self):
        x = self.mean + self.std * torch.randn(self.mean.shape).to(device = self.params.device)
        return x
